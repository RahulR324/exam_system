...
class Base_model extends Model
{
    ...
    public function get_join($joins, $where = [], $select = [], $order_by = null, $limit = null, $group_by = null, $offset = null)
    {
        $query = $this->db->table($this->table);
        if ($select != null) {
            $query->select($select);
    ...
    public function remove($where = null, $remove_db = false)
    {
        ...
        if (!empty($where)) {
            foreach ($where as $column => $value) {
                if (is_array($value)) {
                    if (empty($value)) {
                        $builder->where('1', '0'); // no rows match
                    } else {
