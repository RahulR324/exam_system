<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Add Question - Admin</title>

    <link rel="stylesheet" href="/css/adminstyle.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.13.1/font/bootstrap-icons.min.css"/>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/admin-lte@4.0.0-rc7/dist/css/adminlte.min.css"/>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/katex@0.16.10/dist/katex.min.css">
    <link rel="stylesheet" href="https://taeditor.trogon.info/dist/taeditor.min.css">

    <style>
        .ta-toolbar {
            border-radius: 16px 16px 0 0 !important;
            border: 1px solid #dbe2ea !important;
            border-bottom: none !important;
        }
        .ta-editor {
            border-radius: 0 0 16px 16px !important;
            border: 1px solid #dbe2ea !important;
            min-height: 350px !important;
        }
        .ta-math .katex {
            font-size: 1.1rem !important;
        }
    </style>
</head>

<body class="layout-fixed sidebar-expand-lg sidebar-open">

    <div class="app-wrapper">

        <nav class="app-header navbar navbar-expand bg-body">
            <div class="container-fluid">
                <ul class="navbar-nav">
                    <li class="nav-item">
                        <a class="nav-link" data-lte-toggle="sidebar" href="#"><i class="fas fa-bars"></i></a>
                    </li>
                    <li class="nav-item">
                        <a href="/admin/dashboard" class="nav-link"><i class="fa-solid fa-house"></i> Dashboard</a>
                    </li>
                </ul>

                <div class="admin-dropdown">
                    <button class="admin-btn" onclick="toggleDropdown()">
                        <div class="admin-avatar"><i class="fas fa-user"></i></div>
                        <i class="fas fa-chevron-down"></i>
                    </button>
                    <div class="dropdown-content" id="adminMenu">
                        <div class="admin-info">
                            <div class="admin-avatar small"><i class="fas fa-user"></i></div>
                            <span>Hello Admin</span>
                        </div>
                        <hr>
                        <a href="/admin/logout"><i class="fas fa-sign-out-alt"></i> Logout</a>
                    </div>
                </div>
            </div>
        </nav>

        <aside class="app-sidebar">
            <div class="sidebar-brand">
                <a href="/admin/dashboard" class="brand-link text-decoration-none">
                    <span class="brand-text fw-light">EXAM ADMIN</span>
                </a>
            </div>
            <div class="sidebar-wrapper">
                <nav class="mt-2">
                    <ul class="nav sidebar-menu flex-column" data-lte-toggle="treeview" role="menu">
                        <li class="nav-item"><a href="/admin/dashboard" class="nav-link"><i class="nav-icon fas fa-gauge-high"></i><p>Dashboard</p></a></li>
                        <li class="nav-item"><a href="/admin/course_categories" class="nav-link"><i class="nav-icon fas fa-layer-group"></i><p>Course Categories</p></a></li>
                        <li class="nav-item"><a href="/admin/courses" class="nav-link"><i class="nav-icon fas fa-book-open"></i><p>Courses</p></a></li>
                        <li class="nav-item"><a href="/admin/exams" class="nav-link"><i class="nav-icon fas fa-clipboard-list"></i><p>Exams</p></a></li>
                        <li class="nav-item"><a href="/admin/question_banks" class="nav-link"><i class="nav-icon fas fa-clipboard-question"></i><p>Question Bank</p></a></li>
                        <li class="nav-item"><a href="/admin/view_students" class="nav-link"><i class="nav-icon fas fa-users"></i><p>Students</p></a></li>
                    </ul>
                </nav>
            </div>
        </aside>

        <main class="app-main">
            <div class="main-content container-fluid p-4">

                <div class="category-header mb-4">
                    <div>
                        <h2 class="fw-bold mb-1">Add Question</h2>
                        <p class="text-muted mb-0">Create a new question inside this question bank</p>
                    </div>
                    <a href="<?= base_url('admin/question_banks/'.$questionBank['questionbank_id']) ?>" class="btn btn-outline-secondary rounded-pill px-4">
                        <i class="fas fa-arrow-left me-2"></i> Back
                    </a>
                </div>

                <div class="row justify-content-center">
                    <div class="col-lg-10">
                        <div class="info-card">
                            <div class="mb-4"><h5 class="fw-bold mb-1">Question Details</h5></div>

                            <form method="post" action="<?= base_url('admin/add_question/'.$questionBank['questionbank_id']) ?>">
                                <?= csrf_field(); ?>

                                <div class="mb-4">
                                    <label class="form-label custom-label">Question</label>
                                    <textarea id="question_text" name="question_text"></textarea>
                                </div>

                                <div class="row">
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option A</label>
                                        <input type="text" name="option_a" class="form-control custom-input" placeholder="Enter Option A" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option B</label>
                                        <input type="text" name="option_b" class="form-control custom-input" placeholder="Enter Option B" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option C</label>
                                        <input type="text" name="option_c" class="form-control custom-input" placeholder="Enter Option C" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option D</label>
                                        <input type="text" name="option_d" class="form-control custom-input" placeholder="Enter Option D" required>
                                    </div>
                                </div>

                                <div class="mb-4">
                                    <label class="form-label custom-label">Correct Answer</label>
                                    <select name="correct_answer" class="form-control custom-input" required>
                                        <option value="">Select Correct Option</option>
                                        <option value="A">Option A</option>
                                        <option value="B">Option B</option>
                                        <option value="C">Option C</option>
                                        <option value="D">Option D</option>
                                    </select>
                                </div>

                                <div class="mb-4">
                                    <label class="form-label custom-label">Explanation</label>
                                    <textarea id="explanation" name="explanation"></textarea>
                                </div>

                                <div class="d-flex justify-content-end gap-2">
                                    <a href="<?= base_url('admin/question_banks/'.$questionBank['questionbank_id']) ?>" class="btn btn-light rounded-pill px-4">Cancel</a>
                                    <button type="submit" class="btn btn-outline-primary rounded-pill px-4">
                                        <i class="ti ti-device-floppy me-2"></i> Save Question
                                    </button>
                                </div>
                            </form>
                        </div>
                    </div>
                </div>
            </div>
        </main>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/jquery@3.7.1/dist/jquery.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/admin-lte@4.0.0-rc7/dist/js/adminlte.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/katex@0.16.10/dist/katex.min.js"></script>
    <script src="https://taeditor.trogon.info/dist/taeditor.min.js"></script>
    <script src="/js/admin.js"></script>

    <script>
        TAEditor.create('#question_text', {
            height: 320,
            sanitize: true,
            toolbar: ['bold','italic','underline','|','bulletList','orderedList','|','link','image','table','math','|','clearFormat'],
            math: { katex: window.katex }
        });

        TAEditor.create('#explanation', {
            height: 220,
            sanitize: true,
            toolbar: ['bold','italic','underline','|','bulletList','orderedList','|','math','table'],
            math: { katex: window.katex }
        });
    </script>
</body>
</html>