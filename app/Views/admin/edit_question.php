...
<body class="layout-fixed sidebar-expand-lg sidebar-open">
        ...
        <main class="app-main">
            <div class="main-content container-fluid p-4">
                ...
                <div class="category-header mb-4">
                    <div>
                        <h2 class="fw-bold mb-1">Edit Question</h2>
                        <p class="text-muted mb-0">Update question details and answer configuration</p>
                    </div>
                    <a href="<?= base_url('admin/question_banks/'.$questionBank['questionbank_id']) ?>" class="btn btn-outline-secondary rounded-pill px-4">
                ...
                <div class="row justify-content-center">
                    <div class="col-xl-10">
                        <div class="info-card">
                            <div class="mb-4"><h5 class="fw-bold mb-1">Question Details</h5></div>

                            <form method="post" action="<?= base_url('admin/edit_question/'.$question['question_id']) ?>">
                                <?= csrf_field(); ?>

                                <div class="mb-4">
                                    <label class="form-label custom-label">Question Text</label>
                                    <textarea id="question_text" name="question_text" class="form-control custom-input"><?= esc($question['question_text']) ?></textarea>
                                </div>

                                <div class="row">
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option A</label>
                                        <input type="text" name="option_a" class="form-control custom-input" value="<?= esc($question['option_a']) ?>" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option B</label>
                                        <input type="text" name="option_b" class="form-control custom-input" value="<?= esc($question['option_b']) ?>" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option C</label>
                                        <input type="text" name="option_c" class="form-control custom-input" value="<?= esc($question['option_c']) ?>" required>
                                    </div>
                                    <div class="col-md-6 mb-4">
                                        <label class="form-label custom-label">Option D</label>
                                        <input type="text" name="option_d" class="form-control custom-input" value="<?= esc($question['option_d']) ?>" required>
                                    </div>
                                </div>

                                <div class="mb-4">
                                    <label class="form-label custom-label">Correct Answer</label>
                                    <select name="correct_answer" class="form-select custom-input" required>
                                ...
                                <div class="mb-4">
                                    <label class="form-label custom-label">Explanation</label>
                                    <textarea id="explanation" name="explanation" class="form-control custom-input"><?= esc($question['explanation']) ?></textarea>
                                </div>

                                <div class="d-flex justify-content-end gap-2">
                                    <a href="<?= base_url('admin/question_banks/'.$questionBank['questionbank_id']) ?>" class="btn btn-light rounded-pill px-4">Cancel</a>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            TAEditor.create('#question_text', {
                height: 320,
                sanitize: true,
                toolbar: ['bold', 'italic', 'underline', '|', 'bulletList', 'orderedList', '|', 'link', 'image', 'table', 'math', '|', 'clearFormat'],
                math: { katex: window.katex }
            });

            TAEditor.create('#explanation', {
                height: 220,
                sanitize: true,
                toolbar: ['bold', 'italic', 'underline', '|', 'bulletList', 'orderedList', '|', 'math', 'table'],
                math: { katex: window.katex }
            });
        });
    </script>
